...
ROLLING_WINDOWS = CONFIG.get("rolling_windows", [5])
...
def build_features(seasons: Optional[List[str]] = None) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    elo_df = compute_elo_timeseries(
        df,
        base_rating=float(ELO_CFG.get("base", 1500.0)),
        K=float(ELO_CFG.get("k", 22.0)),
        hfa=float(ELO_CFG.get("home_adv", 60.0)),
    )
    ...
    team_state = (
        pd.merge(elo_snap, long_latest, on="Team", how="outer")
        .sort_values("Team")
        .reset_index(drop=True)
    )
