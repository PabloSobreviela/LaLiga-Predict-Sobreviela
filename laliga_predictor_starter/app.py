...
def vector_for_match(home: str, away: str, feature_order, ts, feature_means: dict):
    ...
    for tcol, (hcol, acol) in mapping.items():
        if hcol in row.index:
            row[hcol] = _ts_value(ts, home, tcol, 0.0)
        if acol in row.index:
            row[acol] = _ts_value(ts, away, tcol, 0.0)
...
with tab_predict:
    ...
    with col_right:
        ...
        use_market = st.toggle(
            "Enable live market odds",
            value=False,
            help="Blend model with normalized bookmaker odds.",
        )
        market_weight = 0.25
        if use_market:
            market_weight = st.slider(
                ...
                0.25,
                0.05,
                help="0 = model only, 1 = market only",
            )
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if use_market:
        ...
        with api_cols[0]:
            api_key_ui = st.text_input(
                "Odds API key (kept local to this session)",
                type="password",
        ...
        with api_cols[2]:
            st.caption(
                "Tip: set `odds_api_key` in `.streamlit/secrets.toml` or `ODDS_API_KEY` env var."
            )
        ...
        changed = (
            home != st.session_state.get("prev_home")
            or away != st.session_state.get("prev_away")
        )
        if (auto_fetch and changed) or fetch_now:
            if api_key:
                result, reason = fetch_live_odds(
                    home, away, api_key, ALIAS_LOOKUP, days_from=90
                )
                if result:
                    oh, od, oa, src = result
                    st.session_state["odds_values"] = {
                ...
                st.info(
                    "No API key set. Enter it above or in secrets/env.", icon="🔑"
                )
        ...
        oddsH = cH.number_input(
            "Odds H",
            min_value=1.01,
            value=float(odds_vals["H"]),
            step=0.01,
            format="%.2f",
        )
        oddsD = cD.number_input(
            "Odds D",
            min_value=1.01,
            value=float(odds_vals["D"]),
            step=0.01,
            format="%.2f",
        )
        oddsA = cA.number_input(
            "Odds A",
            min_value=1.01,
            value=float(odds_vals["A"]),
            step=0.01,
            format="%.2f",
        )
        ...
        if st.session_state["odds_values"].get("src"):
            st.caption(
                f'<span class="pill">Source: {st.session_state["odds_values"]["src"]}</span>',
                unsafe_allow_html=True,
            )
        ...
        with st.expander("Show upcoming fixtures (Odds API names)"):
            ...
            if err:
                ...
                st.caption(
                    "Home vs Away as returned by the API (normalized in italics):"
                )
                for ev in evs:
                    h = ev.get("home_team", "")
                    a = ev.get("away_team", "")
    ...
    if predict_clicked:
        ...
        model_probs = predict_proba_HDA(pipe, X)[0].tolist()
        ...
        st.write({
            "H": round(final_probs[0], 3),
            "D": round(final_probs[1], 3),
            "A": round(final_probs[2], 3),
        })
        ...
        st.caption(
            f"Fair odds →  H: {fair[0]:.2f} • D: {fair[1]:.2f} • A: {fair[2]:.2f}"
        )
...
with tab_train:
    ...
    default_last_n = 3
    default_idx = list(
        range(max(0, len(season_codes) - default_last_n), len(season_codes))
    )
    ...
    if retrain_clicked:
        with st.status("Preparing data…", expanded=True) as status:
            ...
            for s in selected_codes:
                if s == "2526":
                    st.write(
                        f"Note: {season_code_to_label(s)} may not have CSVs yet; training will ignore it if missing."
                    )
                    continue
            ...
            ts, teams_from_file = load_team_state()
            teams_ui = sorted(set(teams_from_file) | OFFICIAL_2526_TEAMS)
            # rebuild alias lookup with updated teams
            ALIAS_LOOKUP = build_alias_lookup(teams_ui)

            status.update(
                label=f"Done → Acc={acc:.3f}, LogLoss={ll:.3f}", state="complete"
            )
