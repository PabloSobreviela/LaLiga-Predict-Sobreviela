# app.py — LaLiga 25/26 Match Predictor (cloud-resilient loading)
...
def load_bundle():
    """
    Resilient model loader: if the model file doesn't exist yet on cloud,
    warn and return placeholders so the UI can ask user to train.
    """
    model_path = Path(CONFIG["model_path"])
    if not model_path.exists():
        st.warning(
            f"Model file not found at **{model_path}**. "
            "Open **🧪 Train / Data** and click **Update dataset & retrain** "
            "to build features and train the model on this server.",
            icon="⚠️"
        )
        return None, None, {}, {}
...
def load_team_state():
    """
    Resilient team_state loader: if missing, return (None, []) and let the UI
    prompt the user to train first.
    """
    fp = DATA_PROC / "team_state.parquet"
    if not fp.exists():
        st.info(
            "Team state not found yet. Go to **🧪 Train / Data** and retrain to generate it.",
            icon="ℹ️"
        )
        return None, []
...
def vector_for_match(home: str, away: str, feature_order, ts, feature_means: dict):
    ...
    if (ts is None) or (home not in ts.index) or (away not in ts.index):
        raise ValueError("Team not found in team_state.parquet. Train the model first.")
...
# --------------------------- LOAD ARTIFACTS (resilient) ---------------------------
pipe, feature_order, feature_means, meta = load_bundle()
ts, teams = load_team_state()
ALIAS_LOOKUP = build_alias_lookup(teams) if teams else {}

# --------------------------- PREDICT ---------------------------
with tab_predict:
    if (pipe is None) or (feature_order is None) or (not teams):
        st.info("Model and team state aren’t ready yet. Go to **🧪 Train / Data** and click **Update dataset & retrain**.", icon="ℹ️")
    else:
        # Card: Match & Options
        ...
        if predict_clicked:
            ...
            model_probs = (
                pipe.predict_proba(X.values)[0].tolist()
                if hasattr(pipe, "predict_proba")
                else pipe.predict_proba(X)[0].tolist()
            )
